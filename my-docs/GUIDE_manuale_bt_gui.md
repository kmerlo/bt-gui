...
- **Commissioni**: o formula libera `lambda q,p: ...` (validazione regex FE + whitelist AST BE che permette `max/min/abs/round` e aritmetica) oppure i 3 parametri `my_commissions_min` (€, floor), `my_commissions_max` (€, cap, vuoto = nessun cap), `my_commissions_perc` (%: 0.1 = 0,1%) — in questo caso si applica `max(MIN, min(MAX, p*abs(q)*perc/100))` con anteprima del costo (es. q=1000, p=100). Ogni modalità ha il suo flag «Applica»: i valori restano precompilabili (default Settings) ma si applicano solo col flag attivo; entrambi i flag = errore. Tutto spento = nessuna commissione. Stessi campi nei default Settings.
