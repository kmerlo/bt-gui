...
**Strategy↔indicator/signal linkage** (dal 2026-09-06, plan `031`): "usato" = ID numerico nei `params` degli algo del tree + liste preset. FE: `bt/utils/collectIds.ts` (unica fonte, riusata da `RunDialog`, `useStrategySave.snapshotIds`, pannelli Saved scoped); al Salva entrambi i campi preset vengono scritti (partizione via `listIndicators`/`listSignals`, fallback tutto in `indicator_source_ids`). BE: `GET /api/bt/usage` (`backend/api/usage.py`, `UsageResponse`) scansiona `strategies.tree_json` e mappa id→nomi strategie (solo IDs esistenti); nessuna migrazione. Rigenerato `frontend/src/types/bt.ts` (+ alias `BuilderPreset`/`UsageResponse` ripristinati dopo `gen:types`, cfr. plan 002).

**Tassazione + zainetto** (dal 2026-09-06, plan `032`): `TaxConfig` in `BacktestConfig` (`enabled`, `default_gain_rate/div_rate` 26%, `use_loss_carry`, `carry_expiry_years=4`, `ticker_rates` snapshot). Post-process puro in `backend/services/tax.py` (`fifo_realized_trades` da `get_transactions()` quantity/price → `apply_tax` → `compute_tax_adjustment`, mai fatal: fallback lordo + `tax_error`): tassa immediata sui gain, zainetto FIFO con scadenza 31/12(anno+4) (`tax_carry_expired_unused`), dividendi (CSV cash totale, gross-up) e coupon (per-unit × posizione ricostruita, no gross-up) sempre tassati mai compensati. Runner salva equity **netta** in `price`, lorda in `price_gross`, `cum_tax`; stats arricchite (`tax_paid_gains/dividends/coupons/total`, `tax_loss_carry_remaining`, `gross_total_return`); transactions +`realized_pnl/trade_tax/carry_after`. Profili globali in tabella `tax_profiles` (seed Default/Governativi/USA), CRUD `GET/POST/PUT/DELETE /api/bt/tax-profiles`, mapping `ticker_tax_profiles` in `RunRequest` risolto in snapshot da `backtest.py`. `GET prices` esclude `price_gross/cum_tax` dalle weights. Tipo sorgente `dividends` aggiunto. FE: `TaxPanel.tsx` (box + mapping, estratto per regola 300 righe), `TaxProfilesPanel.tsx` in Settings, `api/tax.ts`; tipi rigenerati + alias ripristinati (inclusi `TaxConfig/TaxTickerRate/RunRequest`).

**Benchmark buy&hold** (dal 2026-09-05, plan `014-benchmark-buyhold.md`): `POST /api/bt/backtest` accetta `benchmark_ticker` opzionale (normalizzato upper, salvato in `config_json`, default FE `SPY`). Nessuna migration: equity e stats del benchmark sono calcolate on-the-fly in lettura da `backend/services/benchmark.py` (`load_benchmark_series` riusa `_load_prices_from_db`, `align_benchmark_to_index` reindex ffill + scala sul **primo valore equity della strategia** — bt salva l'equity normalizzata base 100, non a initial_capital — `compute_benchmark_stats` su resi giornalieri rf=0/252gg: beta=Cov/Var, alpha ann., correlation, tracking error, information ratio, outperformance TR e CAGR). `GET /runs/{id}` restituisce `benchmark_ticker` + `benchmark` (stats o null); `GET /runs/{id}/prices` accetta `?benchmark_ticker=` (override) e restituisce `benchmark: {ticker, dates, values}` allineato e paginato come la strategia. Benchmark senza dati → `null` morbido, mai 422. FE: `BenchmarkPanel.tsx` + seconda `LineSeries` arancione tratteggiata in `useEquityCharts.ts`; run vecchie senza ticker invariate.

**QuantStats** (dal 2026-09-06): `backend/services/quantstats_service.py` converte l'equity (`prices_parquet`, col `price`) in resi (`pct_change().dropna()`) e chiama `quantstats.stats/plots/reports` (`matplotlib Agg`, lock globale perché pyplot è stateful). Nessuna migration: tutto on-demand con `lru_cache` (metriche/plot 32, tearsheet HTML ~MB solo 8; chiave `(run_id, db_name)`; esistenza run verificata fuori cache così i run cancellati restano 404). Endpoint in `backend/api/runs.py`: `GET /runs/{id}/quantstats` → `{metrics, benchmark_ticker}` (21 metriche extra rispetto a `stats_json` ffn, ognuna guarded → `None`, mai 500); `GET /runs/{id}/quantstats/plot?kind=snapshot|monthly_heatmap|drawdown|distribution` → `{png_base64}` (422 se equity assente/corta); `GET /runs/{id}/quantstats/tearsheet` → HTML (`?download=true` → attachment). Snapshot e tearsheet riusano il benchmark del run (`_benchmark_equity` allineato come `benchmark.py`). FE: `hooks/useQuantStats.ts` (fetch lazy) + `bt/components/QuantStatsPanel.tsx` (card collassabile con 3 tab, iframe solo quando aperto) montato in `ResultsDashboard` dopo `MetricsPanel`. Dip: `quantstats>=0.0.81` + `seaborn` (matplotlib/scipy/tabulate già transitivi via ffn). Test: `tests/backend/test_quantstats.py` (sintetici con `_equity` monkeypatched + 404 + run reale read-only).
...
- **StatInfoRatio con benchmark fuori universo (dal 2026-09-07, fix SPESW_v2/internal error):** in bt originale l'universo conteneva anche il benchmark (`prices` globale con `IVV`), mentre in bt-gui `price_df` = solo i ticker dell'albero (settori). `StatInfoRatio` risolve ora il benchmark in 3 livelli: colonna in `universe` (come prima) → `_benchmark_series` pre-caricata da `backtest_runner.py` (`preload_stat_benchmarks` con stesso `start/end/price_column` del run) → lazy-load da DB (`_load_benchmark_series`, rispettando `get_price_source()` local/market, cache su istanza). Allineamento su indice comune con la window, finestra vuota → stat vuota (cash, no crash). `collect_stat_benchmarks(tree)` (walk puro, no DB) è riusato da `backtest.py` per fail-fast `422` con messaggio azionabile (`Fetch IVV in Ticker Catalog`) invece del generico `internal error` sanitizzato. Il benchmark NON va aggiunto come Security (sporca `SelectAll`).
